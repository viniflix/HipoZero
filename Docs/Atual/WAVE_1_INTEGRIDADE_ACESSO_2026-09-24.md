...
**Data:** 24/09/2026. **Release de código:** `59996f07` (correção) e `1a00614e` (teste de contrato/documentação). **Status:** correção documental publicada; gates operacionais de identidade/email/MFA e teste autenticado ainda abertos.
...
- Vercel mostrou `59996f07` e depois `1a00614e` como **Ready / Production / Current** no domínio `nellonutri.com.br`. `/login` respondeu 200. A Edge nova v1 apareceu `ACTIVE` com `verify_jwt=true`.
...
- As 16 RPCs usadas diretamente pelo gate, serviço e páginas administrativas, incluindo verificações profissionais e solicitações de privacidade, foram listadas no catálogo. Todas negam `EXECUTE` a `anon`. Treze chamam `private.is_admin()` diretamente; as três mutações de verificação profissional chamam `private.require_verification_admin()`, cuja implementação exige `auth.uid()` e `private.is_admin()`. A guarda exige associação não revogada em `private.admin_operators` e JWT AAL2. `sentry-proxy` e `create-patient` consultam `admin_access_status` no caminho administrativo. Isto é uma inspeção de contrato e grants das RPCs usadas pelo frontend, **não** a matriz adversarial completa nem implementação do gateway planejado.
