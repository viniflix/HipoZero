...
4. **Fase 4:** migração `admin20_security_audit`, visão de operadores/MFA e este procedimento. A tela de incidentes passa a chamar a Edge Function por POST e deixa de mostrar saúde/latência fictícias; a área de estudo deixa de instruir uma instalação PostHog inexistente. O release passou em 111 arquivos de teste, 655 testes, lint, build, limite de bundle e auditoria de dependências sem vulnerabilidade alta. Todas as nove RPCs administrativas auditadas negam `EXECUTE` a `anon`. O deploy público final é conferido após o push.
