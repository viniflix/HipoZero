    ...
    <div className="space-y-0">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="flex gap-4 relative">
          ...
          {i !== count - 1 && (
            <div className="absolute left-6 top-10 bottom-[-16px] w-0.5 bg-slate-200" />
          )}
          
          {/* Ícone circular (Timeline Marker) */}
          <div className="relative z-10 shrink-0 mt-2">
            <Skeleton className="w-12 h-12 rounded-full border-4 border-white shadow-sm" />
          </div>
          
          {/* Conteúdo (Timeline Item) */}
