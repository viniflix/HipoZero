...
import { safeExternalHttpUrl } from '@/lib/utils/navigation';
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      {protocols.map((protocol) => {
        const sourceUrl = safeExternalHttpUrl(protocol.source?.url);
        return <Card key={`${protocol.code}-${protocol.version}`} className="flex flex-col">
        ...
        <CardContent className="flex flex-1 flex-col gap-4">
          ...
          <div className="mt-auto flex flex-wrap gap-2"><Button onClick={() => open(protocol)}><BookOpenCheck className="mr-2 h-4 w-4" />AVALIAR PROTOCOLO</Button>{sourceUrl && <Button variant="outline" asChild><a href={sourceUrl} target="_blank" rel="noopener noreferrer"><ExternalLink className="mr-2 h-4 w-4" />VER FONTE</a></Button>}</div>
        </CardContent>
      </Card>;
      })}
    </div>
