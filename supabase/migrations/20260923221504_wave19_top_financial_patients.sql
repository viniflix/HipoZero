-- Aggregate paid revenue in the database; no transaction history leaves the server.
...
as $function$
  ...
  join public.user_profiles up on up.id = ft.patient_id
...
revoke all on function public.get_top_financial_patients(integer) from public, anon;
grant execute on function public.get_top_financial_patients(integer) to authenticated;
