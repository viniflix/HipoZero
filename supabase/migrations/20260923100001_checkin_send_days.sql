-- Wave 14: editable send day and timezone-safe recurring schedule.
...
-- The selected ISO weekday also controls the first and later fortnightly send.
...
begin
  ...
  if p_frequency = 'daily' then
    ...
    for i in case when p_frequency = 'biweekly' then 8 else 1 end
      .. case when p_frequency = 'biweekly' then 14 else 7 end loop
      v_candidate := v_day + i;
    ...
    if v_days[1] not between 1 and 28 then raise exception 'CHECKIN_INVALID_SEND_DAYS'; end if;
