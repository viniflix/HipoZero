...
begin
  ...
  order by
    ...
    case when array_length(v_terms, 1) is null then c.normalized_name end,
    case when array_length(v_terms, 1) is null then (c.food).name end,
    case when array_length(v_terms, 1) is null then 0 else length(c.normalized_name) end,
    (c.food).name
  limit v_limit
  offset v_offset;
end;
