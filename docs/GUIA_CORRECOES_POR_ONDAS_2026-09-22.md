...
Consulta somente leitura ao banco vinculado: 11 cálculos Harris históricos, todos anteriores ao snapshot de mobilidade e ID de injúria. O quociente `GET/(TMB×FI)` é ≈1,20 em 8 registros, ≈1,55 em 2 e ≈1,58 em 1; os últimos 3 não seguem os fatores de mobilidade 1,2/1,3 deste fluxo. Sem saber qual convenção foi usada, não há base para alterar seus resultados. A interface exige revisão antes de reutilizá-los. Não há registro de FI diferente de 1 fora de Harris nos 23 cálculos não Harris consultados.
